...
- Offline (RIB-16): il service worker tiene le immagini viste in due cache CacheFirst con un limite di voci (6.000 miniature, 1.500 immagini grandi; `pwa/runtime-caching.ts`), e l'utente può scaricare in anticipo tutte le immagini di un Set. Le `<img>` usano `crossOrigin="anonymous"` (lo Storage risponde con `Access-Control-Allow-Origin: *`), così la cache salva risposte CORS e non risposte opache, che il browser conterebbe molto più grandi del vero.
